...
require_python() {
	python3 -c "import $1" &> /dev/null || { echo "error: python module $1 not found"; exit 1; }
}
...
then
	...
	# - The downloaded .rpm and .exe files have an embedded signature (which we removed),
	#   locally built files do not so checksums will be different. These files should be
